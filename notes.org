...
#+begin_src dot :results output :exports results :file img/pub-sub-model.png :cmdline -Kdot -Tpng
digraph {
    rankdir=TB;
    ...
    {rank=sink; sub1; sub2; sub3;}

    pub1 [shape=box, label="Publisher"]
    pub_data [shape=box, label="Publisher's data"]
    sub1 [shape=box, label="Subscriber"]
    sub2 [shape=box, label="Subscriber"]
    sub3 [shape=box, label="Subscriber"]

    pub1 -> pub_data;
    pub_data -> sub1 [style=dotted];
    pub_data -> sub2 [style=dotted];
    pub_data -> sub3 [style=dotted];

    labelloc="b";
    label="Fig: Publisher Subscriber Model";
}
#+end_src

#+RESULTS:
[[file:img/pub-sub-model.png]]
...
Say we want to work with some ratings provider(eg: IMDB), which will provide ratings for a movie to us. We need to ingest these ratings along with the rating API that we created earlier. We can make this communication async as we don't need to process this bulk information immediately.
...
For the publisher, let's have an example application that produces rating data and let's have the rating service subscribe to it.

#+begin_src dot :exports results :results output :file img/pub-sub-rating.png :cmdline -Kdot -Tpng
digraph {
    rankdir = TB;
    {rank=source; data_provider};
    {rank=sink; rating_db};

    data_provider [shape=box, label="Data Provider"];
    rating_data [shape=box, label="Rating Data"];
    rating_service [shape=box, label="Rating Service"];
    rating_db [shape=cylinder, label="Rating DB"];

    data_provider -> rating_data [xlabel="Publish"];
    rating_data -> rating_service [style=dotted, xlabel="Subscribe"];
    rating_service -> rating_db;

    labelloc=b;
    label="Fig: Publisher-subscriber model for rating ingestion from data provider"
}
#+end_src

#+RESULTS:
[[file:img/pub-sub-rating.png]]
...
[[./img/kafka-ratings-not-found.png]]
...
[[./img/kafka-rating-ingestion-output.png]]
...
We put some movie metadata and run the =ratingingester= script, which adds rating for movie_id=1, 2 and then call =MovieService/GetMovieDetails= to get the output.

[[./img/grpc-testing-results.png]]

Then we stop the rating service manually and start it again and test the persistence of the stored data.

[[./img/grpc-get-calls-persistance-result.png]]
