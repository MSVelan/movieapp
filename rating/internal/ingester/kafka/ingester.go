...
func NewIngester(addr string, groupID string, topic string) (*Ingester, error) {
	// groupID is used for parallelism, if groupID is same between consumers,
	// then messages will be divided among these two,
	// otherwise full copies of data is provided.
...
func (i *Ingester) Ingest(ctx context.Context) (chan model.RatingEvent, error) {
	...
	go func() {
		for {
			...
			msg, err := i.consumer.ReadMessage(-1) // -1 means consume from beginning.
			if err != nil {
