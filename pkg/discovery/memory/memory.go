...
// New registry creates a new in-memory service registry instance.
...
// ReportHealthyState is a push mechanism for reporting healthy state to a registry.
func (r *Registry) ReportHealthyState(instanceID string, serviceName string) error {
	// We are not really removing the ones which came later than 5 seconds compared to lastActive
	// because the instance could get active again.
	r.Lock()
	defer r.Unlock()
	if _, ok := r.serviceAddrs[serviceName]; !ok {
